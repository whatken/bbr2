...
ssr_link_qr(){
	...
	SSRbase64=$(urlsafe_base64 "${ip}:${port}:${SSRprotocol}:${method}:${SSRobfs}:${SSRPWDbase64}/?obfsparam=www.bing.com&protoparam=")
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: hzbbs):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="hzbbs"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 9888):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="9888"
...
Set_config_method(){
 ...
 ${Green_font_prefix} 3.${Font_color_suffix} aes-256-gcm
 ${Green_font_prefix} 4.${Font_color_suffix} chacha20-ietf-poly1305
...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/makedary/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/makedary/bbaaz/master/other/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/makedary/bbaaz/master/other/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
Install_BBR(){
	[[ ${release} = "Ubuntu" ]] && echo -e "${Error} 本脚本不支持 Ubuntu系统安装 BBR !" && exit 1
	BBR_installation_status
	bash "${BBR_file}"
}
Start_BBR(){
